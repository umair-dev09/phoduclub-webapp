...
function CourseContent() {
    ...
    return (
        <div className="flex flex-col gap-4 ">
            ...
            {/* Drawer for Test */}
            <Test isOpen={showDrawerfortest} toggleDrawer={() => setShowDrawerfortest(!showDrawerfortest)} />
            {/* Drawer for Quiz */}
            <Quiz isOpen={showDrawerforQuiz} toggleDrawer={() => setShowDrawerforQuiz(!showDrawerforQuiz)} />
            {/* Drawer for Video */}
            <Video isOpen={showDrawerforVideo} toggleDrawer={() => setShowDrawerforVideo(!showDrawerforVideo)} />

            {/* content for Accordian of courses */}
            <div className="border border-solid border-[#EAECF0] bg-[#FFFFFF] rounded-md h-auto">
                <Collapsible
                    trigger={
                        <div className="flex flex-row justify-between items-center p-4 w-full ">
                            <div className="flex flex-col gap-1">
                                <span className="font-semibold text-[16px] text-[#1D2939]">Introduction</span>
                                <span className="text-[#667085] font-normal text-sm">0 Lessons</span>
                            </div>
                            <div className="flex flex-row items-center">
                                <button
                                    className="flex flex-row gap-1 items-center rounded-md  h-[44px] w-[112px] justify-center">
                                    <Image src="/icons/plus-sign.svg" height={18} width={18} alt="Plus Sign" />
                                    <span className="text-[#9012FF] font-semibold text-sm">Add Section</span>
                                </button>
                                <Popover
                ...
                >
                    <div className="h-auto flex flex-col w-full">
                        <div className="h-[44px] flex flex-row justify-between bg-[#F2F4F7] px-4 items-center">
                            <span className="text-[#667085] font-medium text-sm">Lessons</span>
                            <div className="flex flex-row gap-20">
                                <span className="text-[#667085] font-medium text-sm">Schedule Date</span>
                                <span className="text-[#667085] font-medium text-sm">Type</span>
                                <span className="text-[#667085] font-medium text-sm">Action</span>

                            </div>
                        </div>
                        <div className="h-[44px] flex flex-row justify-between bg-[#FFFFFF] px-4 items-center rounded-b-md ">
                            <div className="flex flex-row gap-2">
                                <div className="flex items-center justify-center w-[18px] h-[18px] text-sm text-[#475467] font-medium bg-[#EAECF0] border border-lightGrey rounded-[6px]">
                                    1
                                </div>
                                <span className="text-[#1D2939] font-medium text-sm">Welcome Message</span>
                            </div>

                            <div className="flex flex-row gap-20">
                                <span className="text-[#1D2939] font-medium text-sm">24 Jan, 2024  05:30 PM</span>
                                <span className="text-[#1D2939] font-medium text-sm">Text</span>
                                <Popover
                                    placement="bottom-end">
                                    <PopoverTrigger>
                                        <button
