...
const YourComponent = () => {
    // Use a state that can properly store and represent the date
    const [sectionScheduleDate, setSectionScheduleDate] = useState<string | null>(null);

    // Create a controlled value for the DatePicker
    const [dateValue, setDateValue] = useState<any | null>(
        // If you have a previously stored date, parse it
        sectionScheduleDate
            ? parseDate(new Date(sectionScheduleDate).toISOString().split('T')[0])
            : null
    );

    const handleDateChange = (date: any) => {
        // Store the full date object
        setDateValue(date);

        // Convert to a storable string format
        const dateString = date ? date.toString() : null;
        setSectionScheduleDate(dateString);
    };
    // When storing the date
    localStorage.setItem('sectionScheduleDate', dateString);

    // When initializing state
    const storedDate = localStorage.getItem('sectionScheduleDate');
    const initialDateValue = storedDate
        ? parseDate(new Date(storedDate).toISOString().split('T')[0])
        : null;
    return (
        <div>
            jabir
