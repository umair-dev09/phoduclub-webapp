...
function CourseContent() {
    ...
    const [name, setName] = useState("");
    const [sections, setSections] = useState<Section[]>([]); // Specify the type for sections
    ...
    const closeCreateSection = () => {
        if (name.trim()) { // Check if name is not empty
            setSections([...sections, { name }]); // Add new section to sections list
            setName(""); // Clear name input
        }
        setIsCreateSection(false);
    };

    const openDialogForSection = (index: number) => {
    ...
    return (
        <div className="flex flex-col gap-4 ">
            ...
            {/* Display each section dynamically */}
            {sections.map((section, index) => (
                <div key={index} className="bg-[#FFFFFF] h-auto items-center flex flex-col rounded-[16px] border border-solid border-[#EAECF0]">
                    <div className="flex flex-row justify-between items-center p-4 w-full border-b border-solid border-[#EAECF0]">
                        <div className="flex flex-col gap-1">
                            <span className="font-semibold text-[16px] text-[#1D2939]">{section.name}</span>
                            <span className="text-[#667085] font-normal text-sm">0 Lessons</span>
                        </div>
                        <button
                            className="w-10 p-[10px] h-[40px] gap-1 flex-row flex bg-[#FFFFFF] rounded-md shadow-none"
                    ...
                    <div className="flex flex-col p-6 items-center gap-2">
                        ...
                        <div className="flex flex-row gap-4 mt-2">
                            <button
                                onClick={() => openDialogForSection(index)}
                                className="flex flex-row gap-1 items-center rounded-md border-[2px] border-solid border-[#9012FF] h-[44px] w-[162px] justify-center">
                                <Image src="/icons/plus-sign.svg" height={18} width={18} alt="Plus Sign" />
                                <span className="text-[#9012FF] font-semibold text-sm">Add Section</span>
                            </button>
            ...
            <Dialog open={isCreateSection} onClose={() => setIsCreateSection(false)} className="relative z-50">
                <DialogBackdrop className="fixed inset-0 bg-black/30" />
                <div className="fixed inset-0 flex items-center justify-center">
                    <DialogPanel className="bg-white rounded-2xl w-[559px] h-auto">
                        <div className="flex flex-col relative gap-6">
                            <button onClick={() => setIsCreateSection(false)} className="absolute right-4 top-4">
                                <Image src="/icons/cancel.svg" alt="Cancel" width={20} height={20} />
                            </button>
                            <h3 className="mx-6 mt-6 text-2xl font-semibold text-[#1D2939]">Create Section</h3>
                            <div className="flex flex-col w-full gap-2 px-6">
                            ...
                            <div className="flex flex-row justify-end mx-6 my-2 items-center gap-4">
                                ...
                                <button
                                    onClick={closeCreateSection}
                                    className="py-[0.625rem] px-6 rounded-md bg-[#9012FF] font-semibold text-sm text-white">
                                    Create Section
                                </button>
                            </div>
                        </div>
