...
function Video({ isOpen, toggleDrawer, sectionId, courseId, isEditing, contentId }: VideoProps) {
    ...
    const fetchVideoData = async () => {
        ...
        try {
            console.log('Sending request with videocipherId:', videocipherId);
            const response = await fetch('/api/Videocipher', {  // Update to correct endpoint
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ videocipherId }),
            });
