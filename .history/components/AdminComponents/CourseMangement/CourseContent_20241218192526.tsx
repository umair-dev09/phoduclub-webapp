...
function CourseContent({ courseId }: CourseContentProps) {
    ...
    const [selectedDate, setSelectedDate] = useState<Date>(today(getLocalTimeZone())); // Native Date object
    const [selectedTime, setSelectedTime] = useState<TimeValue | null>(null); // TimeValue with hour and minute

    // Convert CalendarDate (if you're using a library like react-calendar) to a native Date
    ...
    const handleTimeChange = (time: { hour: number; minute: number }) => {
        setSelectedTime({
            hour: time.hour,
            minute: time.minute,
            second: 0, // optional, if required by your `TimeValue` type
        });
    };
