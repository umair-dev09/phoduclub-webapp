...
import { parseDateTime, today, getLocalTimeZone } from '@internationalized/date';

const YourComponent = () => {
    // State to store the selected date as a string
    const [sectionScheduleDate, setSectionScheduleDate] = useState<string | null>(() => {
        const storedDate = localStorage.getItem('sectionScheduleDate');
        return storedDate ? storedDate : null;
    });

    // State to manage DatePicker value
    const [dateValue, setDateValue] = useState<any | null>(() => {
        if (sectionScheduleDate) {
            return parseDateTime(new Date(sectionScheduleDate).toISOString()); // Parse full date-time
        }
        return null;
    });

    // Effect to store the selected date in localStorage whenever it changes
    useEffect(() => {
        if (sectionScheduleDate) {
            localStorage.setItem('sectionScheduleDate', sectionScheduleDate);
        }
    }, [sectionScheduleDate]);
    ...
    const handleDateChange = (date: any) => {
        ...
        // Convert the date object to a string and update state
    ...
    return (
        <div>
            ...
            <p>Selected Date: {sectionScheduleDate || 'No date selected'}</p>
            <DatePicker
                granularity="minute"
                minValue={today(getLocalTimeZone())}
                value={dateValue}
