...
const AdminImageCropper = ({ imageFile, setShowCropper, isOpen, setIsOpen }: ImageCropperProps) => {
        ...
        toast.promise(
          new Promise(async (resolve, reject) => {
            try {
              ...
              const storageRef = ref(storage, `AdminProfilePics/${imageFile.name}`);
  ...
  return (
    ...
    //     <DialogPanel transition className='width-[500px] h-auto py-[20px] rounded-md bg-white'>
    //       <>
    //         <div>
    //           <div className='flex flex-row items-center justify-between mx-[24px] mb-[15px] '>
    //             <h3>Crop Image</h3>
    ...
    //             </button>
    //           </div>
    //           <div className='w-auto h-[1px] bg-[#F2F4F7]' />
    //         </div>
    ...
    //   </div>
    // </Dialog>
    <Modal isOpen={isOpen} onOpenChange={(isOpen) => !isOpen && setIsOpen(false)} hideCloseButton >

      <ModalContent>
        <>
          <ModalHeader className="flex flex-row justify-between items-center gap-1">
            <h3 className=" font-bold task-[#1D2939]">Crop Image</h3>
            <button className="w-[32px] h-[32px]  rounded-full flex items-center justify-center transition-all duration-300 ease-in-out hover:bg-[#F2F4F7]">
              <button onClick={() => setIsOpen(false)}>
                <Image src='/icons/cancel.svg' alt="cancel-image" width={18} height={18} />
              </button>
