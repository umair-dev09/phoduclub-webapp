...
function createchannel({ open, onClose }: createchannelProps) {
    ...
    const [isInputValid, setIsInputValid] = useState(false); // For button state
    const [categoryName, setCategoryName] = useState(''); // For input field
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const inputText = e.target.value;
        if (inputText.length <= 100) {
            setCategoryName(inputText);
            setChannelDescription(inputText); // Update state if length <= 100
            setIsInputValid(inputText.trim().length > 0); // Ensures the input isn't just whitespace
        }
    };
            ...
            <div className="fixed inset-0 flex items-center justify-center">
                <DialogPanel className="bg-white rounded-2xl w-[568px] h-auto flex flex-col ">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className='flex px-2 items-center h-[40px] border border-solid border-[#D0D5DD] shadow-sm rounded-md'>
                                ...
                                <input
                                    ...
                                    type="text"
                                    value={categoryName}
                                    onChange={handleInputChange} // Passes the event object directly
                                />
                            </div>
                        </div>
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex px-2 items-center h-[40px] border border-[#D0D5DD] shadow-sm rounded-md">
                                <input
                                    ...
                                    placeholder="Enter channel description" // Updated for better UX
                                    value={channelDescription} // Bind input value to state
                                    onChange={handleInputChange} // Handle input changes
                                />
                            </div>
                            <span className="text-sm text-[#475467] self-end">
                    ...
                    <div className="flex flex-row justify-end mx-6 my-4 gap-4">
                        ...
                        <button
                            className={`py-[0.625rem] px-6 text-white text-sm shadow-inner-button font-semibold ${isInputValid
                                ? "bg-[#9012FF] border border-solid border-[#9012FF]"
                                : "bg-[#CDA0FC] cursor-not-allowed"
                                } rounded-md`}
                            disabled={!isInputValid} // Disables the button if input is invalid
                            onClick={onClose}
