...
function createchannel({ open, onClose }: createchannelProps) {
    ...
    const handleCreateChannelButtonClick = () => {
        // Close the current dialog and show the second one
        sethidedialog(false);  // This will hide the 'create channel' dialog
    };
                    ...
                    <div className="fixed inset-0 flex items-center justify-center">
                        <DialogPanel className="bg-white rounded-2xl w-[568px] h-auto flex flex-col ">
                            ...
                            <div className="flex flex-row justify-end mx-6 my-4 gap-4">
                                ...
                                <button className={`py-[0.625rem] px-6 text-white text-sm shadow-inner-button font-semibold ${uniqueIDfordesc ? "bg-[#9012FF]  border border-solid  border-[#9012FF]" : "bg-[#CDA0FC] cursor-not-allowed"} rounded-md`} onClick={handleCreateChannelButtonClick}>Create Channel</button>
                            </div>
                        </DialogPanel>
                    </div >
                </Dialog >
