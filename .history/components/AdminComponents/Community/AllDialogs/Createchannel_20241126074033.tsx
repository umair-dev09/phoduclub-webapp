                ...
                <DialogPanel className="bg-white rounded-2xl w-[568px] h-auto flex flex-col ">
                    <div className=' flex flex-col p-6 gap-6 border-solid border-[#EAECF0] border-b rounded-t-2xl'>
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className='flex px-2 items-center h-[40px] border border-solid border-[#D0D5DD] shadow-sm rounded-md break-words'>
                                <textarea
                                    className="font-normal text-[#667085] w-full text-sm placeholder:text-[#A1A1A1] rounded-md px-1 py-1 focus:outline-none focus:ring-0 border-none resize-none overflow-hidden"
                                    placeholder="Channel description"
                                    value={uniqueIDfordesc}
                                    rows={2}  // Initial height of 2 rows
                                    onChange={(e) => {
                                        setUniqueIDfordesc(e.target.value);
                                        e.target.style.height = 'auto'; // Reset height to recalculate
                                        e.target.style.height = `${e.target.scrollHeight}px`; // Set height based on scroll height
                                    }}
                                />
                            </div>
                            <span className="font-normal text-sm text-[#475467] flex justify-end">{uniqueIDfordesc.length}/100</span>
                        </div>
