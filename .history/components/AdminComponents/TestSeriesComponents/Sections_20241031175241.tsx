...
//                                     <input type="text" className="w-full px-2 text-sm text-[#182230] font-normal outline-none rounded-md" placeholder="Select Name" />
//                                 </div>
//                             </div>
...
//                                         <input type="text" className="w-full px-2 text-sm text-[#182230] font-normal outline-none rounded-md" placeholder="Select Date" />
//                                     </div>
//                                 </div>
//                                 <div className="flex flex-col w-full gap-2">
...
//                                     <div className="flex flex-row w-full h-10 px-3 outline-none border border-[#D0D5DD] rounded-md">
//                                         <Image src='/icons/calendar-03.svg' alt="date" width={24} height={24} />
//                                         <input type="text" className="w-full px-2 text-sm text-[#182230] font-normal outline-none rounded-md" placeholder="Select Time" />
//                                     </div>
//                                 </div>
//                             </div>
...
//                                 <button className="py-[0.625rem] px-6 text-white shadow-inner-button bg-[#9012FF] border border-[#8501FF] rounded-md   font-semibold text-sm">Create Section</button>
//                             </div>
//                         </div>
//                     </DialogPanel>
//                 </div>
...
function Section() {
    // State to manage dialog visibility for Create Section
    const [isCreateSectionOpen, setIsCreateSectionOpen] = useState(false);

    // State to control content div visibility and store section data
    const [showContent, setShowContent] = useState(false);
    const [sectionData, setSectionData] = useState({ name: "", date: "", time: "" });

    // Handlers for Create Section dialog
    const openCreateSection = () => setIsCreateSectionOpen(true);
    const closeCreateSection = () => setIsCreateSectionOpen(false);

    // Handle form submission in dialog
    const handleCreateSection = () => {
        if (sectionData.name && sectionData.date && sectionData.time) {
            setShowContent(true);
            closeCreateSection();
        }
    };
    ...
    return (
        <>
            <div className="h-auto rounded-[16px] mt-4 flex flex-col border border-solid border-[#EAECF0]">
                <div className="content div h-[76px] bg-[#FFFFFF] rounded-t-[16px] border-b border-solid border-[#EAECF0]">
                    <div className="flex flex-row justify-between p-4">
                        <div className="flex flex-col">
                            <span className="font-semibold text-base text-[#1D2939]">
                                {showContent ? sectionData.name : "Physics"}
                            </span>
                            <div className="flex flex-row mt-2 items-center">
                                <Image src="/icons/select-date.svg" width={16} height={16} alt="Calendar icon" />
                                <span className="font-normal text-[#475467] text-xs ml-1">Schedule :</span>
                                <span className="ml-2 text-[#101828] text-xs font-medium">
                                    {showContent ? `${sectionData.date} ${sectionData.time}` : "24 Jan, 2024 05:30 PM"}
                                </span>
                            </div>
                        </div>
                        <button>
                            <Image src="/icons/three-dots.svg" width={20} height={20} alt="Menu icon" />
                        </button>
                    </div>
                </div>
                ...
                {showContent && (
                    <div className="bg-[#FFFFFF] h-[184px] p-6 items-center flex flex-col gap-2 rounded-b-[16px]">
                        <span className="text-[#1D2939] font-semibold text-lg">Create section/questions</span>
                        <span className="font-normal text-xs text-[#667085]">Your new section content is displayed here.</span>
                    </div>
                )}
                ...
                <div className="bg-[#FFFFFF] h-[184px] p-6 items-center flex flex-col gap-2 rounded-b-[16px]">
                    ...
                    <span className="font-normal text-xs text-[#667085]">Test Lorem ipsum is placeholder text commonly used in the graphic, print, and publishing industries for previewing layouts and visual mockups. Name</span>
                    <div className="flex flex-row gap-4 mt-2">
                        <button className="flex flex-row gap-1 items-center rounded-md border-[2px] border-solid border-[#9012FF] h-[44px] w-[162px] justify-center" onClick={openCreateSection}>
            ...
            <Dialog open={isCreateSectionOpen} onClose={closeCreateSection} className="relative z-50">
                <DialogBackdrop className="fixed inset-0 bg-black/30 " />
                <div className="fixed inset-0 flex items-center justify-center">
                    <DialogPanel transition className="bg-white rounded-2xl w-[559px] h-auto">
                        <div className="flex flex-col relative gap-6">
                            ...
                            <div className="flex flex-col w-full gap-2 px-6">
                                ...
                                <input
                                    type="text"
                                    className="w-full h-10 px-3 text-sm text-[#182230] border border-[#D0D5DD] rounded-md outline-none"
                            ...
                            <div className="flex flex-row justify-between w-full px-6 gap-4">
                                ...
                                <div className="flex flex-col w-full gap-2">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setSectionData({ ...sectionData, time: e.target.value })}
                                    />
                                </div>
                            </div>
