...
function TestQuestions({ questionsList, setQuestionsList }: QuestionsProps) {
    ...
    const handleIndividualCheckboxChange = (index: number) => {
        ...
        if (newSelected.has(index)) {
            ...
            setIsPopoverOpen(true); // Open popover when selecting individually
        }
    ...
    // Custom popover close handler
    const handlePopoverClosecheckbox = (open: any) => {
        // Only allow closing if it's triggered by the main checkbox
        // or if there are no selections
        if (selectedQuestions.size === 0) {
            setIsPopoverOpen(false);
        } else if (!open) {
            // Keep popover open if there are selections
            setIsPopoverOpen(true);
        }
    };

    // Toggle popover specifically for main checkbox click
    const handleMainCheckboxClick = (e: { stopPropagation: () => void; }) => {
        e.stopPropagation();
                        ...
                        <div className="flex flex-row items-center bg-[#F2F4F7] border-b">
                            <div className="w-[5%] pl-4 py-3">
                                <Popover placement="bottom"
                                    ...
                                    onOpenChange={handlePopoverClosecheckbox}>
                                    <PopoverTrigger>
                                        <div onClick={handleMainCheckboxClick}>
                                            <Checkbox
                                                size="md"
                                                color="primary"
