  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const handleKeyPress = (event: KeyboardEvent) => {
        ...
        // Prevent triggering if user is typing in an input field, textarea, or Quill editor
        const isTyping = activeElement?.tagName === "INPUT" || activeElement?.tagName === "TEXTAREA";
        const isQuillFocused = activeElement?.closest(".ql-editor");

        if (event.key === "Enter" && !isTyping && !isQuillFocused) {
          if (isCreateSection) {
            handleAddSection(); // Trigger "Create Section" button
          } else if (deletedialog) {
            if (sectionToDelete) {
              handleDeleteSection(sectionToDelete.id);
            } else if (subsectionToDelete) {
              handleDeleteSubSection(subsectionToDelete.sectionId, subsectionToDelete.parentSectionId);
            }
            setDeletedialog(false);
          } else if (saveQuestionDialog) {
            handleSaveQuestionsWithDetails(saveQuestionSectionId, isUmbrellaTest);
          } else if (csvUploadDialog) {
            document.getElementById("csvUpload")?.click(); // Trigger CSV file input
          }
        }
      };
