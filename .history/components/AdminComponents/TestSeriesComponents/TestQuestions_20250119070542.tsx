...
function TestQuestions({ questionsList, setQuestionsList }: QuestionsProps) {
    ...
    const [selectedQuestions, setSelectedQuestions] = useState([]); // Store selected indices
    const [popoverOpen, setPopoverOpen] = useState(false);

    // Handle main checkbox toggle
    const handleMainCheckboxChange = () => {
        const allSelected = !mainCheckboxChecked;

        setMainCheckboxChecked(allSelected);
        setSelectedQuestions(allSelected ? questionsList.map((_, index) => index) : []);
    };

    // Handle index checkbox toggle
    ...
    // Handle Select All button in the popover
    const handleSelectAll = () => {
        setMainCheckboxChecked(true);
        setSelectedQuestions(questionsList.map((_, index) => index));
    };

    // Handle Unselect All button in the popover
