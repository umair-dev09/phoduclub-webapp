    ...
    return (
        <div className="pb-4 h-auto">
            {questionsList.map((question, index) => (
                <div key={index} className={`rounded-md border mt-4 h-auto bg-[#FFFFFF] ${visited[index] && isDataMissing(question) ? "border-[#FEE4E2]" : "border-[#EAECF0]"}`}>
                    <Collapsible
                        open={openIndex === index}
                        className={`border  rounded-md ${visited[index] && isDataMissing(question) ? "border-[#FEE4E2]" : "border-[#EAECF0]"}`}
                        trigger={
