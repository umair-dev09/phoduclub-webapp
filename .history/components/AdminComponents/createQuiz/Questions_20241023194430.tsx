...
function Questions() {
    ...
    const handleOptionChange = (questionIndex: number, optionKey: keyof Question['options'], e: React.ChangeEvent<HTMLInputElement>) => {
        const newQuestionsList = [...questionsList];
        newQuestionsList[questionIndex].options[optionKey] = e.target.value;
        setQuestionsList(newQuestionsList);
    };
                        ...
                        <div className=' h-auto  bg-[#FFFFFF] flex flex-col pb-5 px-5 gap-2 rounded-br-md rounded-bl-md '>
                            ...
                            <div className="flex flex-col gap-3">
                                {['A', 'B', 'C', 'D'].map((option, idx) => (
                                    <div className="flex flex-row gap-2" key={idx}>
                                        ...
                                        />
                                        <input
                                            className="font-medium pl-3 text-[#101828] text-sm"
                                            placeholder={`Option ${option}`}
                                            value={q.options[option as keyof Question['options']]} // Correct access to options
                                            onChange={(e) => handleOptionChange(index, option as keyof Question['options'], e)} // Proper event handler for options
                                            type="text"
                                        />
                                    </div>
                                ))}
                            ...
                            bg-[#FFFFFF] ${activeQuestionIndex === 0 ? 'shadow-[0px_0px_0px_4px_rgba(158,119,237,0.25),0px_1px_2px_0px_rgba(16,24,40,0.05)]' : ''}`}
                                ...
                                <PopoverContent>
                                    ...
                                    >
                                        {Object.entries(Option).map(([option, value], index) => (
                                            <div
                                                key={index}
                                                className={`flex flex-row justify-between w-full h-[40px] items-center hover:bg-[#F2F4F7] px-2 ${index === Object.keys(option).length - 1 ? 'rounded-bl-md rounded-br-md' : ''}`}
