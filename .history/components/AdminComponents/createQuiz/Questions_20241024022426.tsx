            ...
            {questionsList.map((question, index) => (
                <div key={index} className="rounded-md border border-solid border-[#EAECF0] mt-4 h-auto bg-[#FFFFFF]">
                    <Collapsible
                        ...
                        open={question.isActive} // Control the open state
                        onOpening={() => {
                            // Set isActive true for the currently opening section
                            setQuestionsList(prevList => prevList.map((q, i) => (i === index ? { ...q, isActive: true } : { ...q, isActive: false })));
                        }}
                        onClosing={() => {
                            // Set isActive false for the currently closing section
                            setQuestionsList(prevList => prevList.map((q, i) => (i === index ? { ...q, isActive: false } : q)));
                        }}
                    >
