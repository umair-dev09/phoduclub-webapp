...
function Questions({ questionsList, setQuestionsList }: QuestionsProps) {
    const [openPopoverIndex, setOpenPopoverIndex] = useState<number | null>(null); // State to track which popover is open
    ...
    const handleclickonselectbutton = (index: number) => {
        setOpenPopoverIndex(openPopoverIndex === index ? null : index); // Toggle the popover for the current question
    };
    ...
    return (
        <div className="pb-4 h-auto">
                        ...
                        trigger={
                            <div className='h-auto bg-[#FFFFFF] flex flex-col p-5 gap-2 rounded-md'>
                                <div className="h-auto flex flex-row justify-between">
                                    ...
                                    <Popover placement="bottom-end" isOpen={openPopoverIndex === index} onClose={handlePopoverClose}>
                                        <PopoverTrigger>
                                            <button>
                                                <Image
                                                    src="/icons/three-dots.svg"
                        ...
                        <div className='h-auto bg-[#FFFFFF] flex flex-col pb-5 px-5 gap-2 rounded-br-md rounded-bl-md'>
                            ...
                            {question.isChecked && (
                                <div className="h-36 rounded-xl bg-[#F9FAFB] border-2 border-dashed border-[#D0D5DD]">
                                    <div className="flex flex-col items-center justify-center gap-4 h-full w-full">
                                        ...
                                        <span className="text-sm font-semibold text-[#9012FF]">
                                            Click to upload <span className="font-medium">Image</span>
                                        </span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {['A', 'B', 'C', 'D'].map((optionKey) => (
                                    <div key={optionKey} className="flex flex-row justify-between">
                                        <div className="flex gap-2">
                                            <span className="font-semibold text-lg text-[#1D2939]">{optionKey}.</span>
                                            <input
                                                className="font-medium pl-3 text-[#101828] text-sm placeholder:text-[#A1A1A1] rounded-md placeholder:font-normal
                                            focus:outline-none focus:ring-0 border border-solid border-[#D0D5DD] h-[40px] focus:border-[#D6BBFB] 
                                            focus:shadow-[0px_0px_0px_4px_rgba(158,119,237,0.25),0px_1px_2px_0px_rgba(16,24,40,0.05)]"
                                                ...
                                                type="text"
                                                value={question.options[optionKey as keyof Options]}
                                                onChange={(e) => handleOptionChange(index, optionKey as keyof Options, e.target.value)}
                                            />
                                        </div>
                                        <div className="flex flex-row gap-2">
                                            <input
                                                type="radio"
                                                name={`correctAnswer-${index}`} // Ensure radio buttons are grouped per question
                                                checked={question.correctAnswer === optionKey}
                                                onChange={() => handleCorrectAnswerSelect(index, optionKey as keyof Options)}
                                            />
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs font-normal text-[#5D647D]">{getSelectedAnswerDisplay(question)}</span>
                            </div>
                        </div>
                    </Collapsible>
                </div>
            ...
            <div className="flex justify-end mt-4">
                <button
                    className="px-4 py-2 rounded-md bg-[#A87BB3] text-white font-medium"
                    onClick={handleAddQuestion}
                >
                    Add Question
                </button>
