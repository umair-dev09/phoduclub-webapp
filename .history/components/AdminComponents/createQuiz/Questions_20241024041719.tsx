...
const Questions: React.FC<QuestionsProps> = ({ questionsList: propsQuestionsList, setQuestionsList }) => {
    // Initialize state with proper typing
    const [localQuestionsList, setLocalQuestionsList] = useState<Question[]>(propsQuestionsList);

    // Handler for input change
    const handleInputChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        const newQuestionsList = [...localQuestionsList];
        newQuestionsList[index].question = e.target.value;
        setLocalQuestionsList(newQuestionsList);
    };
    ...
    const handleAddQuestionduplicate = (duplicateQuestion?: Question) => {
        ...
        setLocalQuestionsList([...localQuestionsList, newQuestion]);
    };

    // Handler for deleting question
    const handleDeleteQuestion = (index: number) => {
        console.log("Deleting question at index:", index); // Debugging
        setLocalQuestionsList((prevList) => {
            const updatedList = prevList.map((q, i) => (i === index ? { ...q, isActive: false } : q));
            return updatedList.filter((_, i) => i !== index); // Delete the question
        });
    };
    ...
    return (
        <div className="pb-4 h-auto">
                    ...
                    >
                        <div className='h-auto bg-[#FFFFFF] flex flex-col pb-5 px-5 gap-2 rounded-br-md rounded-bl-md'>
                            ...
                            <div className="flex flex-col gap-3">
                                {(Object.keys(question.options) as Array<keyof Options>).map((optionKey) => (
                                    <div key={optionKey} className="flex flex-row items-center gap-2">
                                        <div className="h-8 w-8 bg-[#F9FAFB] border border-solid border-[#D0D5DD] rounded-[6px] flex items-center justify-center">
                                            <span className="font-semibold text-base text-[#1D2939]">{optionKey}</span>
                                        </div>
                                        <input
                                            type="text"
                                            className="font-medium pl-3 text-[#101828] text-sm placeholder:text-[#A1A1A1] rounded-md placeholder:font-normal
                                    ...
                                    {(Object.keys(question.options) as Array<keyof Options>).map((optionKey) => (
                                        <div key={optionKey} className="flex items-center gap-2">
                                            <input
                                                ...
                                                onChange={() => handleCorrectAnswerSelect(index, optionKey)}
                                            />
                                            <label className="text-[#1D2939]">{optionKey}</label>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <textarea
                                    className="resize-none rounded-md border border-solid border-[#D0D5DD] h-32 placeholder:text-[#A1A1A1] text-[#101828] font-medium"
            ...
            <button onClick={handleAddQuestion} className="mt-4 text-white bg-blue-500 p-2 rounded-md">Add Question</button>
        </div>
    );
};
