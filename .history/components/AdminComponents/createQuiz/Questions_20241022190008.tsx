...
function Questions() {
    ...
    const handleInputChange = (index: number, e: { target: { value: SetStateAction<string>; }; }) => {
        ...
        <div className="pb-4 h-auto">
            {questionsList.map((q, index) => (
                <div key={index} className='mt-2 h-auto rounded-md border border-solid border-[#EAECF0] bg-[#FFFFFF] flex flex-col p-5 gap-2 mb-5'>
                    ...
                    {/* Add more options and correct answer sections here */}
                </div>
            ))}

            <button
                className="mt-4 bg-[#9012FF] text-white px-4 py-2 rounded"
                onClick={handleAddQuestion}
            >
                Add Question
            </button>
