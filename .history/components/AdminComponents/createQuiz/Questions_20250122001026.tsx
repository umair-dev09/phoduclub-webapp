...
function Questions({ questionsList, setQuestionsList }: QuestionsProps) {
    // if any data is missing RED BORDER LINE 
    ...
    const validateQuestions = () => {
        const missing = questionsList.some((question) => {
            return (
                !question.question ||
                !question.options.A ||
                !question.options.B ||
        ...
        setDatamissing(missing); // Update the `datamissing` state based on validation
    };
    return (
        <div className="pb-4 h-auto">
            {questionsList.map((question, index) => (
                <div key={index} className="rounded-md border border-solid border-[#EAECF0] mt-4 h-auto bg-[#FFFFFF]">
                    <Collapsible
                        className={`${datamissing ? "border border-red-500 rounded-md" : "border border-solid border-[#EAECF0] rounded-md"}`}
                        trigger={
                                ...
                                <div
                                    ...
                                    <div className="bg-[#FFFFFF] ">
                                        <ReactQuill
                                            ...
                                            onChange={(content) => { handleQuillChange(index, 'question', content); validateQuestions(); }}
                                            modules={modules}
                                ...
                                {(Object.keys(question.options) as Array<keyof Options>).map((optionKey) => (
                                    <div key={optionKey} className="flex flex-row items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => { handleOptionChange(index, optionKey, e.target.value); validateQuestions(); }}
                                        />
                                    </div>
                                ))}
                            </div>
                            ...
                            >
                                <PopoverTrigger>
                                    <button
                                        ...
                                        onClick={() => { handleclickonselectbutton(index); validateQuestions(); }} // Pass the index
                                    >
                                        <span
                                ...
                                <PopoverContent className="px-0 w-[60.813rem] rounded-md border border-solid border-[#EAECF0] bg-[#FFFFFF] flex flex-col pt-[8px] shadow-lg">
                                    ...
                                    {(Object.keys(question.options) as Array<keyof Options>).map((optionKey) => (
                                        <div
                                            ...
                                            onClick={() => { handleCorrectAnswerSelect(index, optionKey); validateQuestions(); }} // Pass the index
                                        >
                            ...
                            <div
                                ...
                                <div className="bg-[#FFFFFF] ">
                                    <ReactQuill
                                        ...
                                        onChange={(content) => { handleQuillChange(index, 'explanation', content); validateQuestions(); }}
                                        modules={modules}
