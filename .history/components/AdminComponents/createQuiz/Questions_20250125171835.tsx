...
function Questions({ questionsList, setQuestionsList }: QuestionsProps) {
    ...
    const [visited, setVisited] = useState(() => {
        ...
        return storedVisited ? JSON.parse(storedVisited) : Array(questionsList.length).fill(false);
    });

    const [isDataMissing, setIsDataMissing] = useState(() => {
        const storedDataMissing = localStorage.getItem("isDataMissing");
        return storedDataMissing ? JSON.parse(storedDataMissing) : questionsList.map((question) =>
    ...
    // Update localStorage whenever the visited or isDataMissing state changes
    useEffect(() => {
        localStorage.setItem("visited", JSON.stringify(visited));
        localStorage.setItem("isDataMissing", JSON.stringify(isDataMissing));
    }, [visited, isDataMissing]);

    const handleVisitChange = (index: number) => {
        setVisited((prevVisited: any) => {
    ...
    return (
        <div className="pb-4 h-auto">
            {questionsList.map((question, index) => (
                <div key={index} className={` ${visited[index] && isDataMissing(index) ? "border-1.5 border-[#F04438]" : " border border-[#EAECF0]"} rounded-md   mt-4 h-auto bg-[#FFFFFF] `}>
                    ...
                    <Collapsible
                        ...
                        onOpening={() => handleVisitChange(index)}
                    >
