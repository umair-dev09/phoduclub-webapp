...
function Questions({ questionsList, setQuestionsList }: QuestionsProps) {
    ...
    const handleOptionSelect = (questionIndex: number, optionKey: keyof Options) => {
        setSelectedAnswer(optionKey as string); // Update selected answer with the correct type
        handlePopoverClose(); // Close the popover when an option is selected
    };
