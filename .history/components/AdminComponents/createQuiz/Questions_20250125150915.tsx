        ...
        <div className="pb-4 h-auto">
            {questionsList.map((question, index) => (
                <div
                    ...
                    className={` 
                   ${!visited[index]
                            ? "border border-gray-400" // Grey border for first-time visits
                            : isDataMissing(question)
                                ? "border-1.5 border-[#F04438]" // Red border if data is missing
                                : "border border-[#EAECF0]"} // Default border
                   rounded-md mt-4 h-auto bg-[#FFFFFF]  
               `}
                    onClick={() => handleOpen(index)} // Ensure visiting is tracked when clicked
