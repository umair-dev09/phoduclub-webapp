...
function Questions({ questionsList, setQuestionsList }: QuestionsProps) {
    ...
    const checkMissingFields = (questionData: any): boolean => {
        return !questionData.question || !questionData.correctAnswer;
    };
    ...
    const handleCorrectAnswerSelect = (questionIndex: number, optionKey: keyof Options) => {
        ...
        setMissing(checkMissingFields(newQuestionsList[questionIndex]));
    };
        ...
        <div className="pb-4 h-auto">
            {questionsList.map((question, index) => (
                <div key={index} className="rounded-md border border-solid border-[#EAECF0] mt-4 h-auto bg-[#FFFFFF]">
                    <Collapsible
                        className={`${missing ? "border border-red-500 rounded-md" : "border border-solid border-[#EAECF0] rounded-md"}`}
                        trigger={
