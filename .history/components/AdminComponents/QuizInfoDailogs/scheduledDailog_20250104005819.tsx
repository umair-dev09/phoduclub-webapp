            ...
            <ModalContent>
                <>
                    ...
                    <ModalFooter className="border-t border-lightGrey">
                        <Button variant="light" className="py-[0.625rem] px-6 border-[1.5px] border-lightGrey text-[#1D2939] hover:bg-[#F2F4F7] font-semibold text-sm rounded-md" onClick={onClose}>Cancel</Button>
                        <Button onClick={onSchedule}
                            className={`h-[44px] w-auto px-3 ml-4 rounded-md text-white font-medium items-center flex border border-solid border-white  ${!isFormValid ? 'bg-[#CDA0FC]' : 'bg-[#9012FF] hover:bg-[#6D0DCC] '} justify-center shadow-inner-button`}
                            disabled={!isFormValid}>Schedule {fromContent === "testseries"
                                ? "Testseries"
                                : fromContent === "quiz"
                                    ? "Quiz"
