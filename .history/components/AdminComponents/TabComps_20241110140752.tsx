...
const TooltipButton: React.FC<TooltipButtonProps> = ({
    ...
    <div className="relative group">
        <button
            onClick={onClick}
            className={`flex w-full py-2 px-3 text-base text-left font-normal rounded-md mb-2 transition-all ${isActive ? 'bg-[#7400E0] text-white' : 'hover:bg-[#e1ffe11a] text-[#AAAAAA]'
                }`}
        >
            <Image src={isActive ? activeIcon : icon} width={22} height={22} alt={`${label} Icon`} />
            {!isCollapsed && <span className='ml-2'>{label}</span>}
        ...
        {isCollapsed && (
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-4 px-4 py-2 bg-[#131313] text-white text-sm rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 whitespace-nowrap z-10">
                {label}
            </div>
        )}
    </div>
...
function TabComps() {
    ...
    const isContentSection = () => activeTab.includes('content');

    // Modified content section component
    ...
    return (
        <div className={`flex flex-col relative transition-all duration-[500ms] ease-in-out ${isCollapsed ? 'w-[3.5rem]' : 'w-[16rem]'
            } pl-2 py-[0.625rem] bg-[#131313]`}>
            {/* Sidebar toggle button */}
            <div className="flex justify-end pr-2 mb-4">
                <button onClick={() => setIsCollapsed(!isCollapsed)}>
                    <Image
                        src={isCollapsed ? '/icons/menu-open.svg' : '/icons/menu-close.svg'}
                        width={20}
                        height={20}
                        alt="Toggle Collapse"
                    />
            ...
            <TooltipButton
                label="Dashboard"
                icon="/icons/admin-dashboard-2.svg"
                activeIcon="/icons/admin-dashboard.svg"
                isActive={activeTab === 'dashboard'}
                isCollapsed={isCollapsed}
                onClick={() => handleTabClick('dashboard', '/admin/dashboard')}
            />

            {/* Content section with tooltips */}
            <TooltipButton
                label="content"
                icon="/icons/admin-dashboard-2.svg"
                activeIcon="/icons/admin-dashboard.svg"
                ...
                isCollapsed={isCollapsed}
                onClick={() => handleTabClick('content', '/admin/content')}
            />

            {/* Additional tabs with tooltips */}
            <TooltipButton
                label="Role Management"
                icon="/icons/Role Management-2.svg"
                activeIcon="/icons/Role Management.svg"
                isActive={activeTab === 'rolemanagement'}
                isCollapsed={isCollapsed}
            ...
            <TooltipButton
                ...
                icon="/icons/Marketing Integration-2.svg"
                activeIcon="/icons/Marketing Integration.svg"
                isActive={activeTab === 'marketingintegration'}
                isCollapsed={isCollapsed}
            ...
            <TooltipButton
                ...
                activeIcon="/icons/community.svg"
                isActive={activeTab === 'customerdatamanagement'}
                isCollapsed={isCollapsed}
