...
function DeleteCourse({ open, onClose, courseId, courseName }: DeleteCourseProps) {
    ...
    useEffect(() => {
        const handleKeyPress = (event: KeyboardEvent) => {
            if (event.key === "Enter" && isFormValid) {
                handleDeleteCourse(); // ✅ Trigger delete if input is valid
            }
        };
    ...
    }, [isFormValid, handleDeleteCourse]);
        ...
        <Modal isOpen={open} onOpenChange={(isOpen) => !isOpen && onClose()} hideCloseButton >
            <ModalContent>
                <>
                    ...
                    <ModalBody >
                        <span className="font-normal text-sm text-[#667085]">All data inside the course will be gone.mmmmmmmmmmmmmm</span>
