...
function StudentsPurchasedCourseInfo({ courseId }: StudentsAttemptsProps) {
    ...
    useEffect(() => {
        const loadStudentsAttempts = async () => {
            setLoading(true);
            try {
                const attemptsRef = collection(db, 'course', courseId, 'StudentsPurchased');
    ...
    const handleAddUser = async (userId: string) => {
        try {
            ...
            // Check if user already exists in StudentsPurchased
            const studentRef = doc(db, 'course', courseId, 'StudentsPurchased', userDoc.id);
            const studentDoc = await getDocs(collection(db, 'course', courseId, 'StudentsPurchased'));
            const exists = studentDoc.docs.some(doc => doc.id === userDoc.id);
                            ...
                            {data.length > 0 ? (
                                currentItems.map((students, index) => (
                                    <tr key={index} className="border-t border-solid border-[#EAECF0]">
                                            ...
                                            <div className="flex flex-row ml-8 gap-2">
                                                <div className="flex items-center">
                                                    <div className="relative">
                                                        <Image className="rounded-full w-10 h-10" src={'/images/DP_Lion.svg'} alt="DP" width={40} height={40} />
                                                ...
                                                <div className="flex items-start justify-start flex-col">
                                                    <div className="font-semibold">{"phodu user"}</div>
                                                    <div className="flex justify-start items-start text-[13px] text-[#667085]">{"phodu id"}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-4 text-center text-[#101828] text-sm">45%</td>
                ...
                <ModalContent>
                    <>
                        <ModalHeader className="flex flex-row justify-between items-center gap-1">
                            <h3 className=" font-bold task-[#1D2939]">Remove user from this course?</h3>
                            <button
                                className="w-[32px] h-[32px] rounded-full flex items-center justify-center transition-all duration-300 ease-in-out hover:bg-[#F2F4F7]"
                        ...
                        <ModalBody >
                            <p className="pb-2 text-sm font-normal text-[#667085]"> Are you sure you want to remove this user from the course? This action cannot be undone.</p>
                        </ModalBody>
