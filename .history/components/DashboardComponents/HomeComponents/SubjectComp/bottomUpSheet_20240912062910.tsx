...
const BottomSheet: React.FC<BottomUpSheet> = ({ closeModal, subjectName }) => {
    ...
    const CustomCalendarContainer = ({ className, children }: any) => (
        ...
        //     </div>
        // </div>
        <div className={`${className} bg-gray-100 border border-gray-300 shadow-lg rounded-lg p-4 flex flex-col h-full`}>
            {/* Calendar content */}
            ...
            <div className="flex justify-between mt-4">
                <button onClick={cancelDate} className="bg-customwhite text-black px-4 py-2 rounded-md">Cancel</button>
                <button onClick={applyDate} className="bg-custompurple text-white px-4 py-2 rounded-md">Apply</button>
            </div>
        </div>
    );
