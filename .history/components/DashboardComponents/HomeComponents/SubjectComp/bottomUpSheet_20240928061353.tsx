...
//             <div
//                 className={`${isOpen ? (isClosing ? 'animate-slideDown' : 'animate-slideUp') : ''
//                     } transition-transform duration-500 ease-in-out z-50 ${styles.bottomUpSheet} ${isOpen ? styles.open : ''}`}
//             >
...
const BottomSheet: React.FC<BottomUpSheet> = ({ closeModal, subjectName }) => {
    ...
    const CustomCalendarContainer = ({ className, children }: any) => (
        <div className={`${className} relative`}>
            ...
            <div className="flex justify-between mt-2">
                <button onClick={cancelDate} className="px-4 py-2 bg-gray-200 rounded-lg">Cancel</button>
                <button onClick={applyDate} className="px-4 py-2 bg-purple-600 text-white rounded-lg">Apply</button>
            </div>
        </div>
    );
                    ...
                    <tr key={rowIndex} className='h-[84px]'>
                        <td>
                            ...
                            <div className='flex items-center ml-6'>
                                <progress value={getProgress(rowIndex)} max="100" className="w-1/2 h-2 bg-gray-100 rounded-full" />
                                <div className="ml-2 text-sm font-normal">
                                    <p>{getProgress(rowIndex)}%</p>
                                </div>
                            </div>
                        ...
                        <td>
                            <div className="inline-flex items-center justify-center border border-gray-300 rounded-full bg-white text-sm font-medium text-gray-700 p-2 w-[67px] h-[28px]">
                                <span className="w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                                <p className="m-0 text-sm text-gray-700">{getPriorityText(chapterName)}</p>
                            </div>
                        </td>
                        <td>
                            <DatePicker
                                ...
                                onCalendarClose={() => setCalendarVisible(false)}
                                dateFormat="dd-MM-yyyy"
                                className="w-[110px] text-center border border-gray-300 rounded-md"
                            />
                        </td>
                        {checkboxes[rowIndex].map((checked, index) => (
    ...
    return (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-end justify-center z-10">
            <div id="bottomSheet"
                className={`fixed bottom-0 left-0 grid grid-rows-[69px_1fr_76px] w-full h-[98vh] bg-white rounded-t-lg transform ${isOpen ? (isClosing ? 'animate-slide-down' : 'animate-slide-up') : ''} transition-transform duration-500`}
            >
                <div className="flex items-center justify-between rounded-t-lg px-6">
                ...
                <div className="overflow-auto">
                    <table className="w-full table-fixed border-collapse">
                        <thead className="bg-gray-100">
                            <tr className='text-xs'>
                                <th className="text-left px-6">Chapter</th>
                                <th>Priority</th>
                                <th>Target Date</th>
                                <th>Theory</th>
                                <th>Practice</th>
                ...
                <div className="flex items-center justify-end border-t border-gray-200 p-4">
                    <button className="border rounded-lg py-2.5 px-6 text-sm border-gray-200 text-gray-700" onClick={CloseModal}>
                        Cancel
                    </button>
                    <button className="ml-2 border rounded-lg py-2.5 px-6 text-sm bg-purple-600 text-white">
                        Save
                    </button>
                </div>
            </div>
