...
const BottomSheet: React.FC<BottomUpSheet> = ({ closeModal, subjectName }) => {
    ...
    const openBottomSheet = () => {
        ...
        setIsOpening(true); // Trigger opening animation
        setTimeout(() => {
            setIsOpen(true); // After animation, show the modal
            setIsOpening(false); // Reset opening state
        }, 500); // Match duration with your opening animation
    };
    useEffect(() => {
        openBottomSheet(); // Automatically open the sheet when the component mounts
    }, []);
    ...
    return (
        <div className={styles.container}>
            <div
                id="bottomSheet"
                className={`${styles.bottomUpSheet} ${isOpening ? styles.opening : ''} ${isClosing ? styles.closing : ''}`}
            >
