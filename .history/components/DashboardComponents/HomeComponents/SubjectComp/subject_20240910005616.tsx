...
const SubjectLayout: React.FC = () => {
    ...
    const handleButtonClick = (subjectName: string) => {
        console.log(`${subjectName} button clicked`);
        setSelectedSubject(subjectName); // Store the selected subject
    };
    ...
    return (
        <div className={styles.container}>
            ...
            <BottomSheet
                ...
                onDismiss={() => setOpen(false)}
            >
                <h2>Popup Content for {selectedSubject}</h2>
                <button onClick={() => setOpen(false)}>Close</button>
            </BottomSheet>
        </div>
    );
