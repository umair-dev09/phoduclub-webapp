...
const BottomSheet: React.FC<BottomUpSheet> = ({ closeModal, subjectName }) => {
    ...
    const CustomCalendarContainer = ({ className, children }: any) => (
        <div className={`${className} ${styles.customCalendarContainer}`}>
            {children}
            <div className={styles.customButtons}>
                <button onClick={cancelDate} className={`${styles.cancelButton} cancel`}>Cancel</button>
                <button onClick={applyDate} className={`${styles.applyButton} apply`}>Apply</button>
            </div>
        </div>
    );
            ...
            case 'Chemistry':
                return (
                    <tr className='h-[84px]'>
                        ...
                        <td>
                            <div className="inline-flex items-center justify-center border border-gray-300 rounded-full bg-white text-sm font-medium text-gray-700 p-2">
                                <span className="w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                                <p className="m-0 text-sm text-gray-700">High</p>
                            </div>
                        </td>
