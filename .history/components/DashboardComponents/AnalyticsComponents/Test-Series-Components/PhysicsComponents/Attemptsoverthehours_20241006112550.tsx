...
const CustomTick: React.FC<CustomTickProps> = ({ x, y, payload, index, dataLength }) => {
    ...
    // Check if it's the first, middle, or last label and adjust position accordingly
    if (index === 0) {
        positionX = x + 100;  // Adjust for first label
    } else if (index === dataLength - 1) {
        positionX = x - 50;  // Adjust for last label (changed from x - 50 to x + 50)
    } else {
        positionX = x;  // Keep the default position for middle labels
    }
