...
Welcome to the channel`;
    ...
    const [activeButtons, setActiveButtons] = useState(Array(10).fill(false)); // Change 10 to the number of buttons you need

    const handleClick = (index) => {
        setActiveButtons((prev) =>
            prev.map((active, i) => (i === index ? !active : active)) // Toggle the active state for the clicked button
        );
    };

    return (
        <div className="mx-6 h-full  mt-6 flex flex-col gap-6">
            {/* First message */}
            ...
            <div className="w-full h-auto flex flex-col">
                ...
                <div className="flex flex-wrap mt-1 gap-2 h-auto ml-11">
                    {activeButtons.map((isActive, index) => (
                        <button
                            key={index}
                            onClick={() => handleClick(index)}
                            className={`rounded-[54px] border border-solid border-[#D0D5DD] bg-[#F2F4F7] h-[26px] w-[48px] p-1 flex flex-row justify-center gap-1 ${isActive
                                    ? 'bg-[#F8F0FF] border-[#7400E0]' // Active styles
                                    : 'hover:bg-[#F8F0FF] hover:border-[#7400E0]' // Hover styles
                                }`}
                        >
                            <Image
                                src="/icons/emoji-5.png"
                    ...
                    <button className=" rounded-[54px] border border-solid border-[#D0D5DD] bg-[#F2F4F7] h-[26px] w-[48px] p-1    flex flex-row justify-center gap-1">
                        <Image
                            src="/icons/emoji-5.png"
                            width={15}
                            height={15}
