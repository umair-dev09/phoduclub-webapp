...
function Learned() {
    ...
    const handleTabClick = (tabName: string, path: string) => {
        setActiveTab(tabName);
        router.push(path); // Ensure absolute paths
        console.log("the button are clicked")
    };

    useEffect(() => {
        if (pathname) {
            const currentPath = pathname.split("/")[2]; // Adjusted to split on the first slash
            if (currentPath === "test") {
                setActiveTab("test");
            } else if (currentPath === "Quiz") {
                setActiveTab("Quiz");
            ...
            <div className="flex w-full flex-col">
                <Tabs
                    ...
                    classNames={{
                        ...
                        tab: "max-w-fit px-0 h-12 focus:outline-none focus:ring-0",
                        tabContent: "group-data-[selected=true]:font-bold",
                    }}
                >
                    <Tab
                        key="courses"
                        onClick={() => handleTabClick("courses", "/learn/courses")} // Fixed the path
                        title={
                    ...
                    <Tab
                        key="test"
                        onClick={() => handleTabClick("test", "/learn/test")} // Fixed the path
                        title={
                    ...
                    <Tab
                        key="Quiz"
                        onClick={() => handleTabClick("Quiz", "/learn/Quiz")} // Fixed the path
                        title={
                            <div className="flex items-center space-x-2">
                                <span
