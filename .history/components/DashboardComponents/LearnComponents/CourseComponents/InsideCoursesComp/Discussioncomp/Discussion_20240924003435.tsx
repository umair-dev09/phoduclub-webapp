...
function Discussion() {
    ...
    const handleIconClick = (format: string) => {
        if (quill) {
            quill.format(format, true); // Ensure quill instance is not null before calling format
        }
    };

    useEffect(() => {
        if (quillRef.current) {
            // Type assertion that quillRef.current is not null and has getEditor method
            ...
            <div className="mr-[24px] mt-[20px] ml-[24px] bg-[#F7F8FB] border border-solid border-[#EAECF0] rounded-[12px] h-auto">
                ...
                <div className="bg-[#F7F8FB] border-b border-solid border-b-[#EAECF0] rounded-tl-[12px] rounded-tr-[12px]">
                    <ReactQuill
                        ...
                        className="bg-transparent text-[#1D2939] focus:outline-none rounded-b-[12px]  custom-quill" // Add custom class
                        style={{
                    ...
                    />
                    {value === '' && (
                        <div className="absolute top-3 left-4 text-[#667085] italic" style={{ fontFamily: 'Poppins, sans-serif' }}>
                ...
                <div className="h-[66px] bg-[#F7F8FB] rounded-bl-[12px] rounded-br-[12px] flex justify-center items-center">
                    <div className="flex flex-row w-full justify-between items-center mx-5">
                        ...
                        <div className="h-[24px] w-[288px] gap-[24px] flex flex-row">
                            {/* Icons */}
                            <button>
                                <div onClick={() => handleIconClick('bold')}
                                >
