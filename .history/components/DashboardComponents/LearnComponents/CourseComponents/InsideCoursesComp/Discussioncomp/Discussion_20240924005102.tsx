...
function Discussion() {
    ...
    const handleIconClick = (format: string) => {
        if (quill) {
            const isBold = quill.getFormat().bold; // Check if bold is currently applied
            quill.format('bold', !isBold); // Toggle bold formatting
        }
    };
    ...
    // This will clear the bold formatting when the user types
    const handleKeyDown = () => {
        if (quill) {
