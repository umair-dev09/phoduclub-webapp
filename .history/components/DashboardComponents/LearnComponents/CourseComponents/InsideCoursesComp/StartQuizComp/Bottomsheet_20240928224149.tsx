...
function Quiz({ isOpen, setIsOpen }: QuizProps) {
    ...
    const closeBottomSheet = () => {
        ...
        // Wait for the animation to finish before hiding the bottom sheet
        setTimeout(() => {
            setIsClosing(false); // End the closing animation
        }, 500); // Match this duration with the CSS animation duration

        // Hide the bottom sheet after the animation completes
        setTimeout(() => {
            setShowBottomSheet(false); // Actually hide the bottom sheet
        }, 510); // Delay slightly longer than the animation duration to ensure smooth hiding
    };
    useEffect(() => {
    ...
    return (
        <>
            ...
            {(showBottomSheet || isClosing) && (
                <div
                    className={`fixed bottom-0 left-0 w-full h-[98vh] bg-white rounded-t-2xl transform 
