...
function QuizAttendingArea({
    ...
    useEffect(() => {
        const handleKeyPress = (event: KeyboardEvent) => {
            if (event.key === "Enter") {
                if (isOpen) {
                    openBottomSheet(); // Trigger openBottomSheet if isOpen is true
                } else if (isDialogOpen) {
                    handleDialogSubmit(); // Trigger handleDialogSubmit if isDialogOpen is true
                }
            }
        };
    ...
    }, [isOpen, isDialogOpen, openBottomSheet, handleDialogSubmit]);
