...
function SideButton() {
    ...
    return (
        <div className="absolute top-0 right-0 w-[363px] h-full bg-[#FFFFFF] border border-b border-r">
            <div className="bg-[#FFFFFF]" style={{ borderRight: '1px solid #EAECF0' }}>
                <div className="w-[331px] rounded-tl-lg m-3" style={{ paddingLeft: "10px", borderRadius: '8px 0 0 0' }}>
                    <div className="flex flex-col h-full">
                        {['bit2.1', 'bit2.2', 'bit2.3'].map((tab, index) => (
                            <button
                                key={index}
                                onClick={() => handleTabClick(tab, `/bit2/${tab}`)}
                                className={`w-full h-[70px] bg-[#F8F0FF] text-[#1D2939] text-base font-medium rounded-lg flex items-start ${activeTab === tab ? 'text-base font-bold bg-[#F8F0FF]' : ''}`}
                            >
                                <div className="flex-1 flex items-center flex-col">
                                    <div className="flex items-center flex-1">
                                        <Image
                                            src={iconClicked ? '/icons/Green-tick.svg' : (activeTab === tab ? '/icons/course-circle.svg' : '/icons/course-circle2.svg')}
                                            width={20}
                                            height={20}
                                            alt="circle-course"
                                            style={{ marginTop: "9px", marginLeft: '15px' }}
                                        ...
                                        />
                                        <span
                                            className={`flex ${activeTab === tab ? 'font-bold' : 'font-normal'} text-[#000000]`}
                                            style={{ marginTop: "9px", marginLeft: '15px' }}
                                        >
