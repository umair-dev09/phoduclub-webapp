...
function TabComps() {
    ...
    return (
        <div className={`flex flex-col relative transition-all duration-[500ms] ease-in-out ${isCollapsed ? 'w-[3.5rem]' : 'w-[16rem]'} pl-2 py-[0.625rem] bg-[#131313]`}>
            ...
            <button className={`flex items-center justify-center absolute top-[3.6rem] left-[97.5%] w-8 h-8 z-10 bg-white border-[0.106rem] border-lightGrey transition-all duration-300 ease-in-out hover:bg-[#F2F4F7]" rounded-full  ${isCollapsed ? ' -translate-x-1' : ''}`} onClick={handleCollapseClick}>
                {isCollapsed ? (
