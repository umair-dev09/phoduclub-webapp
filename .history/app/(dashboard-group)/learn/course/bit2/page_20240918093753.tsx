...
export default function Bit2() {
    // State to manage each button's state
    const [buttons, setButtons] = useState([
        { id: 'bit2.1', path: '/bit2/bit2.1', text: '1. Welcome and Introduction', time: '10:10', iconClicked: false, isActive: false },
        { id: 'bit2.2', path: '/bit2/bit2.2', text: '2. Chapter 01 Introduction', time: '9:20', iconClicked: false, isActive: false },
    ...
    const handleButtonClick = (index: number) => {
        setButtons(buttons.map((button, i) =>
            i === index
                ? { ...button, isActive: true } // Set the clicked button as active
                : { ...button, isActive: false } // Deactivate all other buttons
        ));
    };
    ...
    const handleIconClick = (index: number) => {
        setButtons(buttons.map((button, i) =>
            i === index
                ? { ...button, iconClicked: !button.iconClicked } // Toggle the clicked button's icon
                : { ...button } // Keep other buttons unchanged
        ));
    };
    ...
    return (
        <div className="relative flex flex-col flex-1 pb-8">
            <div className="absolute top-0 right-0 w-[363px] h-[581px] bg-[#FFFFFF] border border-b border-r">
                <div className="h-[480px] bg-[#FFFFFF]" style={{ borderRight: '1px solid #EAECF0' }}>
                    {[
                        { id: 'bit2.1', path: '/bit2/bit2.1', text: '1. Welcome and Introduction', time: '10:10' },
                        { id: 'bit2.2', path: '/bit2/bit2.2', text: '2. Chapter 01 Introduction', time: '9:20' },
                        { id: 'bit2.3', path: '/bit2/bit2.3', text: '3. Chapter 01 heading', time: '11:45' }
                        // Add more button objects as needed
                    ].map((button) => (
                        <div key={button.id} className="flex flex-col h-full justify-start ">
                            <button
                                onClick={() => handleButtonClick(index)}
                                className={`w-full h-[70px] text-[#1D2939] text-base font-medium rounded-lg flex items-start ${button.isActive ? 'bg-[#F8F0FF]' : 'bg-[#FFFFFF]' // Background changes based on active state
                                    }`}
                            >
                                <div className="flex-1 flex items-center flex-col">
                                    <div className="flex items-center flex-1">
                                        {/* Image logic based on state */}
                                        <Image
                                            src={
                                                button.iconClicked
                                                    ? '/icons/Green-tick.svg' // Green tick if icon clicked
                                                    : button.isActive
                                                        ? '/icons/course-circle2.svg' // Active state image
                                                        : '/icons/course-circle.svg' // Default image
                                            }
                                            width={20}
                                            height={20}
                                            alt="circle-course"
                                            ...
                                            onClick={(event) => {
                                                ...
                                                handleIconClick(index); // Toggle iconClicked state
                                            }}
                                        />
                                        <span
                                            className={`flex ${button.isActive ? 'text-base font-bold text-[#000000]' : 'text-base text-[#1D2939]'} `}
                                            style={{ marginTop: "9px", marginLeft: '15px' }}
                                        >
                                            {button.text}
                                        </span>
                                    ...
                                    <div className="flex items-center flex-1" style={{ marginTop: "6px" }}>
                                        <Image
                                            ...
                                            className="ml-5"
                                        />
                                        <span className="flex font-normal text-[#667085]" style={{ marginLeft: '10px' }}>
                                            {button.time}
                                        </span>
                                    </div>
                                </div>
