...
export default function Bit2() {
    ...
    // Handles button click event to change active tab
    const handleButtonClick = (tabName: string, path: string) => {
        setActiveTab(tabName);
        setIconStates(prev => ({ ...prev, [tabName]: !prev[tabName] }));
        router.push(path);
    };

    // Handles icon click event to toggle icon state
    const handleIconClick = (tabName: string, event: React.MouseEvent) => {
    ...
    return (
        <div className="relative flex flex-col flex-1 pb-8">
            <div className="absolute top-0 right-0 w-[363px] h-[581px] bg-[#FFFFFF] border border-b border-r">
                <div className="flex flex-col h-full justify-start p-4"> {/* Added padding for overall spacing */}
                    {[
                        ...
                        <div key={button.id} className="mb-3"> {/* Added bottom margin to ensure space between buttons */}
                            <button
                                onClick={() => handleButtonClick(button.id, button.path)}
                                className={`w-full h-[70px] flex items-center rounded-lg ${activeTab === button.id ? 'bg-[#F8F0FF]' : 'bg-[#FFFFFF]'
                                    }`}
                                style={{ marginBottom: '8px' }} // Ensure spacing between buttons
                            >
                                <div className="flex-1 flex items-center">
                                    <Image
                                        src={
                                            iconStates[button.id]
                                                ? '/icons/Green-tick.svg' // Green tick when pressed
                                                : (activeTab === button.id
                                                    ? '/icons/course-circle2.svg' // Active button
                                                    : '/icons/course-circle.svg') // Inactive button
                                        }
                                        width={20}
                                        height={20}
                                        alt="course-icon"
                                        ...
                                        onClick={(event) => handleIconClick(button.id, event)}
                                    />
                                    <span
                                        className={`ml-3 ${activeTab === button.id ? 'text-base font-bold text-[#000000]' : 'text-base text-[#1D2939]'}`}
                                    >
                                        {button.text}
                                    </span>
                                </div>
                                <div className="flex items-center ml-auto mr-5">
                                    <Image
                                        ...
                                        alt="course-learn"
                                    />
                                    <span className="ml-2 font-normal text-[#667085]">{button.time}</span>
                                </div>
                            </button>
                        </div>
                    ))}
