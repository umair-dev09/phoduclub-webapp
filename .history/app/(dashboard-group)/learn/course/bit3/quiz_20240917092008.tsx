...
function Quiz() {
    ...
    return (
        isOpen && ( // Conditionally render the dialog only when `isOpen` is true
            <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
                <div className="fixed inset-0 bg-black/30" onClick={() => setIsOpen(false)} /> {/* Added onClick to the overlay */}
                <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                    <DialogPanel className="bg-white rounded-2xl p-5 w-[500px]" onClick={(e) => e.stopPropagation()}> {/* Prevents closing when clicking inside the dialog */}
