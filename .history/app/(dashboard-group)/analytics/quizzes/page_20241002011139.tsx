...
interface CustomTooltipProps extends TooltipProps<number, string> {
    ...
    coordinate?: { x: number; y: number }; // Optional coordinate prop
}

const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label, coordinate }) => {
    if (active && payload && payload.length && coordinate) {
        ...
        // Get the tooltip position or default to (0, 0)
        const tooltipX = coordinate.x;
        const tooltipY = coordinate.y;

        // Calculate tooltip position to the left of the bar
        const tooltipStyle: React.CSSProperties = {
            position: 'absolute',
            left: tooltipX - 70, // Shift to the left (adjust for width)
            top: tooltipY - 20,  // Adjust vertical position above the bar
        ...
        return (
            <div style={tooltipStyle}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '5px' }}>
                    ...
                    <span>{`Correct: ${correctValue}`}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <span>{`Incorrect: ${incorrectValue}`}</span>
                </div>
            </div>
        );
    }
            ...
            <div className="flex w-full h-auto flex-row gap-4">
                <div className="w-1/2 rounded-xl h-[300px] flex-col">
                    <BarChart width={400} height={350} data={data}>
                        ...
                        <XAxis dataKey="name" />
                        <YAxis domain={[0, 'dataMax + 1']} /> {/* Set domain for Y-axis */}
