...
export default function DashboardGroup({ children }: DashboardGroupProps) {
    const pathname = usePathname(); // Get the current path, e.g., "/math-quiz"
    const searchParams = useSearchParams(); // Get query parameters like "?qId=123"
    const [currentPage, setCurrentPage] = useState<string>('');

    useEffect(() => {
        const pathArray = pathname?.split('/'); // Split the path by "/"
        const dynamicSegment = pathArray?.[pathArray.length - 1] || ''; // Get the last segment
        const qId = searchParams?.get('qId'); // Extract query parameter 'qId'

        let pageName = '';

        // Handle dynamic routes (like quizinfo dynamic segment)
        if (pathname?.includes('quizinfo')) {
            pageName = Back to Quizzes Management;
        } else if (dynamicSegment) {
            // Customize based on dynamic segment
            if (qId) {
                pageName = Quiz: ${ dynamicSegment }; // Show dynamic quiz name
            } else {
                pageName = Back to ${ dynamicSegment.replace(/-/g, ' ') }; // Replace hyphens with spaces for better readability
            }
        } else {
            ...
            pageName = routeMapping[dynamicSegment] || 'Dashboard';
        }
