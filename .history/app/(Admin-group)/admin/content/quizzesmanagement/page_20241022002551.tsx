...
const itemsPerPage = 2; // Rows per page (2 rows per page for 20 pages total)

// Generate 50 rows of dummy data
const data = Array.from({ length: 50 }, (_, index) => ({
    quiz: `Maths ${index + 1}`,
    questions: 10,
    published: `Jan ${6 + Math.floor(index / 5)}, 2024`,
    students: 2147 + (index * 10), // Vary the number of students for each row
    status: ["Saved", "Live", "Cancelled", "Paused", "Scheduled"][(index % 5)], // Alternate statuses
}));
            ...
            <div className="flex flex-row justify-between items-center">
                ...
                <div className="flex flex-row gap-3">
                    ...
                    <button
                        ...
                        onClick={() =>
                            router.push("/admin/content/quizzesmanagement/createquiz")
                        }
                    >
                        <span className="text-[#FFFFFF] font-semibold text-sm">Create Quiz</span>
                    </button>
                ...
                <table className="w-full bg-white rounded-xl">
                    <thead>
                        <tr>
                            ...
                            <th className="w-[17%] text-center px-8 py-4 pr-8 rounded-tr-xl text-[#667085] font-medium text-sm">
                                Status
                            </th>
                    ...
                    <tbody>
                        {currentItems.map((item, index) => (
                            <tr key={index}>
                                <td className="w-1/4 text-left px-8 py-4 pl-8 border-t border-t-lightGrey text-[#9012FF] font-semibold text-sm">
                                    {item.quiz}
                                </td>
                                <td className="w-[17%] text-center px-8 py-4 border-t border-t-lightGrey text-[#1D2939] font-normal text-sm">
                                    {item.questions}
                                ...
                                <td className="w-[17%] text-center px-8 py-4 border-t border-t-lightGrey text-[#1D2939] font-normal text-sm">
                                    <div className="flex justify-center">
                                        <Image
                                            src={`/icons/${item.status.toLowerCase()}.svg`}
                                            width={74}
                                            height={24}
                                            alt={item.status}
                                        />
