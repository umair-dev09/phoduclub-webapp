...
function CreateQuiz() {
    ...
    const canProceedToNext = () => {
        if (currentStep === Step.QuizInfo) {
            // Both name and description must be filled
            return quizName.trim() !== '' && quizDescription.trim() !== '';
        }
        else if (currentStep === Step.Publish) {
            return marksPerQ !== 0 &&
                ...
                timeNumber.trim() !== '' &&
                timeText.trim() !== '' &&
                (!isPremiumQuiz || (isPremiumQuiz && product !== null));
        }
        return questionsList.every(question =>
            question.question.trim() !== '' &&
            question.options.A.trim() !== '' &&
    ...
    useEffect(() => {
        ...
        // Add event listener
        document.addEventListener('keydown', (event: KeyboardEvent) => handleKeyPress(event));

        // Cleanup
        return () => {
            document.removeEventListener('keydown', (event: KeyboardEvent) => handleKeyPress(event));
        };
    }, [isNextButtonDisabled, currentStep, quizName, quizDescription, questionsList, marksPerQ, endDate, timeNumber, timeText, isPremiumQuiz, product]);
