...
function CreateQuiz() {
    ...
    useEffect(() => {
        const handleKeyPress = (event: KeyboardEvent) => {
            ...
            // Ensure both input fields are filled before allowing Enter
            const isQuillEmpty = !quizDescription || quizDescription.trim() === "" || quizDescription.trim() === "<p><br></p>";
            const isInputEmpty = !quizName || quizName.trim() === "";
            const isBothFilled = !isInputEmpty && !isQuillEmpty;

            if (event.key === "Enter") {
                if (!isNextButtonDisabled && isBothFilled) {
                    handleNextClick(); // ✅ Move to next only if both fields are filled
                } else if (isQuillFocused) {
                    if (isQuillEmpty) {
                        event.preventDefault(); // ❌ Prevent submission when Quill is empty
                    }
                }
            }
        };
    ...
    }, [isNextButtonDisabled, quizName, quizDescription]);
