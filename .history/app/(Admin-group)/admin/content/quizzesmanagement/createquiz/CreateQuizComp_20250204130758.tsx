...
function CreateQuiz() {
    ...
    useEffect(() => {
        const handleKeyPress = (event: KeyboardEvent) => {
            ...
            // Check if Quill editor is focused
            const isQuillFocused = activeElement?.closest(".ql-editor");

            // Ensure both fields are filled before allowing Enter to submit
            const isBothFilled = quizName.trim() !== "" && quizDescription.trim() !== "" && quizDescription.trim() !== "<p><br></p>";

            if (event.key === "Enter") {
                if (!isNextButtonDisabled && isBothFilled) {
                    handleNextClick();
                } else if (!isBothFilled && isQuillFocused) {
