...
function CreateQuiz() {
    ...
    const [quizCreated, setQuizCreated] = useState(false); // New state to handle quiz creation

    const handleNextClick = () => {
        if (currentStep < 3) {
            setCurrentStep(currentStep + 1);
        } else if (currentStep === 3) {
            // When "Publish" is clicked, set quizCreated to true
            setQuizCreated(true);
        }
    };
    ...
    const renderStepContent = () => {
        if (quizCreated) {
            return <QuizCreated />; // Show QuizCreated component if quiz is created
        }
    ...
    return (
        <>
            {!quizCreated && ( // Hide this section when quiz is created
            ...
            <div className="flex flex-col w-full ml-[20px] mr-8 mt-8">
                <div className={`h-15 ml-1 w-full ${quizCreated ? "" : "border-b border-solid border-[#D0D5DD]"}`}>
