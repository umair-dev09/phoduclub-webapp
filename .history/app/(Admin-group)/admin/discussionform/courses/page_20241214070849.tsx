...
function Courses() {
    ...
    const [quillKey, setQuillKey] = useState(0); // Key to force Quill remount

    // Memoized Quill modules to prevent unnecessary re-renders
    ...
    useEffect(() => {
        // Force Quill to remount when switching tabs
        setQuillKey(prev => prev + 1);
    }, [activeTab]);

    // Initialize Quill when it's rendered
    useEffect(() => {
        if (quillRef.current) {
            setQuill(quillRef.current.getEditor());
        }
    }, []); // Re-run initialization when tab changes
