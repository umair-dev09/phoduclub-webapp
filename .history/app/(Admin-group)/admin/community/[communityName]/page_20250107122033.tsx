...
function Chatinfo() {
  ...
  const handlePopoverOpen = (headingId: string) => {
    setActivePopover(Number(headingId));
    const id = Number(headingId);
    console.log("Opening popover for headingId:", id); // Debug log
    setActivePopover(id);
  };
              ...
              <div key={heading.headingId} className="ChannelHeadingDiv w-full h-auto mb-[13px]">
                <div className="flex flex-row justify-between items-center mb-[12px] px-2">
                  ...
                  <Popover placement="bottom-start"
                    isOpen={activePopover === Number(heading.headingId)} // Use headingId for comparison
                    onOpenChange={(open) => open ? handlePopoverOpen(heading.headingId) : setActivePopover(null)} // Pass headingId
                  >
                    <PopoverTrigger>
