...
export default function PaymentStatusPage() {
    ...
    const storeDataInFirestore = async () => {
      ...
      if (uid){
        ...
        try {
          ...
          // Only redirect for non-mobile devices
          if (deviceType !== 'mobile') {
            if (productType === 'course') {
              router.replace(`/learn/courses`);
            } else if (productType === 'testseries') {
  ...
  return (
    <div className="p-6 w-screen mx-auto flex items-center justify-center h-screen flex-col gap-2">
      ...
      {deviceType === 'mobile' ? (
        <p className='italic text-center'>Transaction complete. You can close this window now.</p>
      ) : (
        <p className='italic text-center'>Redirecting you to the product page. If not redirected please close this tab.</p>
      )}
    </div>
  );
}