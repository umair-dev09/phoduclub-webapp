                ...
                <div className="flex flex-col px-3 pb-6 mt-4 gap-2 overflow-y-auto">
                    {receivedRequestsCount >= 1 && (
                        <>
                            <button className="flex flex-row items-center justify-between w-full h-auto px-3 py-3 rounded-md bg-white hover:bg-[#F8F0FF]"
                                onClick={() => router.push('/community/private-chat/new-requests')}>
                    ...
                    {chatUsers.sort((a, b) => {
                        ...
                        <button key={index} 
                            ...
                            onClick={() => {
                                router.push(`/community/private-chat/${user.name.toLowerCase().replace(/\s+/g, '-')}/?pId=${generateChatId(currentUserId || '', user.uniqueId)}&admin=${user.isAdmin}`);
