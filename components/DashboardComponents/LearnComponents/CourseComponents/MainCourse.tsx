"use client";

import Image from "next/image";
import styles from "./maincourse.module.css";

function main() {
    return (
        <div className="flex flex-1 flex-row">
            
            {/* ----------- course component ----------- */}
            <div className={styles.course1}>
                <div className="flex flex-1  h-[60%] items-center flex-col">
                    <div className={styles.suggestion}>
                        <Image
                            className={styles.suggestionIcon}
                            src="/icons/suggestion_icon.svg"
                            alt="suggestion icon"
                            width={16}
                            height={16}
                        />
                        <p>Suggested for you</p>
                    </div>
                    <div>
                        <img src="/images/course_img.svg" alt="Course" width={300} height={300}/>
                    </div>
                </div>

                {/* Wrap the two courseInfo divs in a flex container */}
                {/* Added flex-row and gap-x */}
                <div className={styles.courseInfo}>
                    <div className="flex h-[60%] items-center flex-col">
                        <div className={styles.courseName}>
                            <div>
                                <p>BITSET Full Course</p>
                            </div>
                            <div>
                                <Image alt='Collapse Icon Right' src="/icons/collapse-right.svg" width={8} height={8} />
                            </div>
                        </div>
                        <div className={styles.courseLength}>
                            <p>3 Lessons</p>
                            <span>&#x2022;</span> {/* Bullet point */}
                            <p>3hr 14m</p>
                        </div>
                    </div>

                    <div className="flex h-[40%] flex-col">
                        <div className="flex relative w-full h-2 rounded-full bg-gray-200 mb-1">
                            <div
                                className="absolute top-0 left-0 h-2 rounded-full bg-progressPurple"
                                style={{ width: "43%" }}
                            ></div>
                        </div>

                        <div className="flex flex-1 flex-row justify-between mt-2 text-xs">
                            <div className="flex flex-row gap-1">Completed: <span className="font-m">43%</span></div>
                            <div className="flex flex-row gap-1">Time Left: <span className="font-m">28 days left</span></div>
                        </div>
                    </div>
                </div>
            </div> {/* End of flex-row div */}

            {/* ----------- course component ----------- */}
            <div className={styles.course1}>
                <div className="flex flex-1  h-[60%] items-center flex-col">
                    <div className={styles.suggestion}>
                        <Image
                            className={styles.suggestionIcon}
                            src="/icons/suggestion_icon.svg"
                            alt="suggestion icon"
                            width={16}
                            height={16}
                        />
                        <p>Suggested for you</p>
                    </div>
                    <div>
                        <img src="/images/course_img.svg" alt="Course" width={300} height={300}/>
                    </div>
                </div>

                {/* Wrap the two courseInfo divs in a flex container */}
                {/* Added flex-row and gap-x */}
                <div className={styles.courseInfo}>
                    <div className="flex h-[60%] items-center flex-col">
                        <div className={styles.courseName}>
                            <div>
                                <p>BITSET Full Course</p>
                            </div>
                            <div>
                                <Image alt='Collapse Icon Right' src="/icons/collapse-right.svg" width={8} height={8} />
                            </div>
                        </div>
                        <div className={styles.courseLength}>
                            <p>3 Lessons</p>
                            <span>&#x2022;</span> {/* Bullet point */}
                            <p>3hr 14m</p>
                        </div>
                    </div>

                    <div className="flex h-[40%] flex-col">
                        <div className="flex relative w-full h-2 rounded-full bg-gray-200 mb-1">
                            <div
                                className="absolute top-0 left-0 h-2 rounded-full bg-progressPurple"
                                style={{ width: "43%" }}
                            ></div>
                        </div>

                        <div className="flex flex-1 flex-row justify-between mt-2 text-xs">
                            <div className="flex flex-row gap-1">Completed: <span className="font-m">43%</span></div>
                            <div className="flex flex-row gap-1">Time Left: <span className="font-m">28 days left</span></div>
                        </div>
                    </div>
                </div>
            </div> {/* End of flex-row div */}

            {/* ----------- course component ----------- */}
            <div className={styles.course1}>
                <div className="flex flex-1  h-[60%] items-center flex-col">
                    <div className={styles.suggestion}>
                        <Image
                            className={styles.suggestionIcon}
                            src="/icons/suggestion_icon.svg"
                            alt="suggestion icon"
                            width={16}
                            height={16}
                        />
                        <p>Suggested for you</p>
                    </div>
                    <div>
                        <img src="/images/course_img.svg" alt="Course" width={300} height={300}/>
                    </div>
                </div>

                {/* Wrap the two courseInfo divs in a flex container */}
                {/* Added flex-row and gap-x */}
                <div className={styles.courseInfo}>
                    <div className="flex h-[60%] items-center flex-col">
                        <div className={styles.courseName}>
                            <div>
                                <p>BITSET Full Course</p>
                            </div>
                            <div>
                                <Image alt='Collapse Icon Right' src="/icons/collapse-right.svg" width={8} height={8} />
                            </div>
                        </div>
                        <div className={styles.courseLength}>
                            <p>3 Lessons</p>
                            <span>&#x2022;</span> {/* Bullet point */}
                            <p>3hr 14m</p>
                        </div>
                    </div>

                    <div className="flex h-[40%] flex-col">
                        <div className="flex relative w-full h-2 rounded-full bg-gray-200 mb-1">
                            <div
                                className="absolute top-0 left-0 h-2 rounded-full bg-progressPurple"
                                style={{ width: "43%" }}
                            ></div>
                        </div>

                        <div className="flex flex-1 flex-row justify-between mt-2 text-xs">
                            <div className="flex flex-row gap-1">Completed: <span className="font-m">43%</span></div>
                            <div className="flex flex-row gap-1">Time Left: <span className="font-m">28 days left</span></div>
                        </div>
                    </div>
                </div>
            </div> {/* End of flex-row div */}
        </div>

    );
}

export default main;
