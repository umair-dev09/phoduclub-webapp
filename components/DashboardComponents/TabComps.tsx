...
function TabComps() {
    ...
    return (
        <div className={`flex flex-col relative transition-all duration-[500ms] ease-in-out ${isCollapsed ? 'w-[3.5rem]' : 'w-[16rem]'} pl-2 py-[0.625rem] bg-[#131313]`}>
            ...
            <button className={`flex items-center justify-center absolute top-[3.6rem] left-[97.5%] w-8 h-8 z-10 bg-white border-[0.106rem] border-lightGrey transition-all duration-300 ease-in-out hover:bg-[#F9FAFB] rounded-full " ${isCollapsed ? ' -translate-x-1' : ''}`} onClick={handleCollapseClick}>
                {isCollapsed ? (
            ...
            <div className='overflow-hidden'>
                ...
                <div className={`flex-col mt-2 mb-[0.475rem] ml-2 transition-all ${!isCollapsed ? 'flex' : 'hidden'}`}>
                    <button onClick={() => { router.push('/dashboard') }} className='text-left text-white text-lg font-bold mt-3 mb-2'>phodu<span className='text-pink text-lg font-bold'>.club</span></button>
                </div>
            </div>
