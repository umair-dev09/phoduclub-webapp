...
function OtherChat({ message, currentUserId, mentions, isDeleted, highlightedText, messageType, fileUrl, fileName, isHighlighted, scrollToReply, fileSize, senderId, timestamp, channelId, chatId, isReplying, replyingToId, replyingToChatId, replyingToFileName, replyingToFileUrl, replyingToMsg, replyingToMsgType, setShowReplyLayout, handleReply }: OtherChatProps) {
  const [reactions, setReactions] = useState<ReactionCount[]>([]);
  const [isOpen, setIsOpen] = useState(false);
  const [showBookmark, setShowBookmark] = useState(false); // Use a single index to track the active button
  const [showMediaDialog, setShowMediaDialog] = useState(false); // Use a single index to track the active button
  ...
  const reactionsRef = useMemo(() => {
    return collection(
      db,
      `internalchat/${channelId}/chats/${chatId}/reactions`
    );
  }, [channelId, chatId]);
  ...
  const handleAddReaction = async (emoji: EmojiClickData) => {
    try {
      ...
      const reactionsRef = doc(db, `internalchat/${channelId}/chats/${chatId}/reactions`, userId);
  ...
  const handleDeleteReaction = async (emoji: string) => {
    try {
      ...
      const userId = currentUser.uid;
      const reactionDocRef = doc(db, `internalchat/${channelId}/chats/${chatId}/reactions`, userId);
