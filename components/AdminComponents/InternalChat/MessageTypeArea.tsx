...
function MessageTypeArea({
  ...
  const handleSend = async () => {
    if ((!text.trim() && !fileUrl) || !internalChatId|| !channelId) {
  ...
  const handleMediaUpload = async (file: File, type: "image" | "video" | "document") => {
    if (!internalChatId  || !channelId) return;
  
    try {
      const storageRef = ref(storage, `InternalChatUploads/${internalChatId}/${channelId}/${file.name}`);
